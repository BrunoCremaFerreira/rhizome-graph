# Plan: File-size colour mode (F7) -- assessment and staged plan

- **Status:** done -- R1-R9 implemented; R10-R14 not built (R10 was ranked "next", the rest
  "noted"; each carries its trigger in section 3)
- **Created:** 2026-08-25 22:17
- **Implemented:** 2026-08-25 (branch `development`, fast-forwarded into `main`)
- **PR/commit:** `2be23a7` (backend), `7b71de6` (frontend); `b18cf48` recorded this plan
- **Consultations (mandatory):** `software-architect` (2026-08-25) -- this document is its
  assessment and staged plan, and it names the owner of every RED/GREEN step below.
  `security-auditor` is referred to twice inside the plan; it was not consulted.

Written 2026-08-25 against `3a0bd01`, with both suites green at the numbers in section 0. Every
...
nothing in it knows the paths came from a query. `2026-08-23-02-51-content-search.md` reused it for a second search
...
- **A refused command is reported as a `rootError` painted in the observed-root bar.**
  `server.py:847-865` and `main.ts:275`. `2026-08-23-02-51-content-search.md` filed this as R11 (noted) when a
...
`2026-08-23-02-51-content-search.md` applies to its own R10.
...
- one entry in the keys legend, `web/index.html:24-27`. **That file IS scanned by the language
  ...
  new string is English-checked automatically -- the gap `2026-08-23-02-51-content-search.md` filed as its R13 has
...
`2026-08-17-16-21-multi-repo-git-status.md`: the plan carries the divergence, rather than the code carrying a cast
...
This is the `R12` of `2026-08-23-02-51-content-search.md` in a new place, and the same reasoning applies: the name
...
every `rootError` in the observed-root bar. `2026-08-23-02-51-content-search.md` filed this as its R11 when a fourth
...
This is `2026-08-23-02-51-content-search.md`'s R14 with a smaller constant (~500 ms against ~3 s), and it inherits
